/*
Table Descriptives - Monetary Policy and the Wage-Inflation Unemployment Trade-off
...
esttab using "$hp\Output\Tables\Table_Descriptives.tex", cells("count(fmt(0)) mean(fmt(2)) sd(fmt(2)) min(fmt(2)) max(fmt(2))") noobs nonumber label replace ///
