...
foreach x in lunemp {	
	foreach y in lwage {
		...
		graph export "$hp\Output\Figures\fig_`p`j''_SDPMBM_LPIV`horizon2'_`k'_asym`what'.pdf", replace
...
graph export "$hp\Output\Figures\fig_`p`j''_SDLPIVBM`horizon2'_`k'_asym`what'.pdf", replace
}
}
}
...
* open log file
cap log cl
log using "../Output/Tables/Table_asym`what'_`horizon2'_`match'.tex", t replace
set linesize 255
